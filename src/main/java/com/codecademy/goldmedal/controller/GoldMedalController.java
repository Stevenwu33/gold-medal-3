...
public class GoldMedalController {
    ...
    @GetMapping("/{country}")
    public CountryDetailsResponse getCountryDetails(@PathVariable String country) {  //returns all info from country
        String countryName = WordUtils.capitalizeFully(country);
        return getCountryDetailsResponse(countryName); // this returns the Method "getCDR" located at the bottom
    }
    ...
    private CountryDetailsResponse getCountryDetailsResponse(String countryName) { //gets all the details of a country by user passing in a country
        var countryOptional = countryRepository.getByName(countryName); //this custom Q gets the name of the country from the Repo
        if (countryOptional.isEmpty()) {
            return new CountryDetailsResponse(countryName);  //if the input value is empty, we create a new page, but empty
        }

        var country = countryOptional.get();
        var goldMedalCount = goldMedalRepository.countByCountry(countryName); //with the user country input, we get the number of gold medal inputted in our db with that country
        ...
        var totalSummerEvents = goldMedalRepository.countBySeason("Summer"); //counts a ll summer wins by all countries
        var percentageTotalSummerWins = totalSummerEvents != 0 && numberSummerWins != null ? (float) summerWins.size() / totalSummerEvents : null;
        var yearFirstSummerWin = summerWins.size() > 0 ? summerWins.get(0).getYear() : null; //0 is the first input in the list
